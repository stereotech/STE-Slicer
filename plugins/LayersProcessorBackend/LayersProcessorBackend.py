...
class LayersProcessorBackend(QObject, Backend):
    backendError = Signal()

    def __init__(self) -> None:
        super().__init__()
        executable_name = "LayersProcessor"
        ...
        if not default_engine_location:
            raise EnvironmentError("Could not find LayersProcessor")

        Logger.log("i", "Found LayersProcessor at: %s", default_engine_location)

        default_engine_location = os.path.abspath(default_engine_location)
        self._application.getPreferences().addPreference("surface_backend/location", default_engine_location)
        ...
        self._process_mesh_job = None # type: Optional[ProcessMeshJob]
    ...
    def getGlicerEngineCommand(self) -> List[str]:
        executable_name = "glicer_surface_console.exe"
        default_engine_location = executable_name
        if os.path.exists(os.path.join(SteSlicerApplication.getInstallPrefix(), "bin", executable_name)):
            default_engine_location = os.path.join(SteSlicerApplication.getInstallPrefix(), "bin", executable_name)
        if hasattr(sys, "frozen"):
        ...
        if not default_engine_location:
            raise EnvironmentError("Could not find Glicer Surface")

        Logger.log("i", "Found Glicer Surface at: %s", default_engine_location)
        default_engine_location = os.path.abspath(default_engine_location)
        return [default_engine_location]

    def getEngineCommand(self) -> List[str]:
        command = [self._application.getPreferences().getValue("surface_backend/location"), "connect", "127.0.0.1:{0}".format(self._port), ""]
    ...
    def slice(self) -> None:
        ...
        slice_message = self._socket.createMessage("layersprocessor.proto.Slice")
    ...
    def _onStartSliceCompleted(self, job: StartSliceJob) -> None:
        ...
        self._startProcessMeshLayersJob(output_path, self._application.getMultiBuildPlateModel().activeBuildPlate, arcus_message)
    ...
    def _startProcessMeshLayersJob(self, output_path: List[str], build_plate_number: int, arcus_message: Arcus.PythonMessage) -> None:
    ...
    def _onProcessMeshFinished(self, job: ProcessMeshJob):
        self._message_handlers["layersprocessor.proto.LayerOptimized"] = self._onOptimizedLayerMessage
        self._message_handlers["layersprocessor.proto.Progress"] = self._onProgressMessage
        self._message_handlers["layersprocessor.proto.GCodeLayer"] = self._onGCodeLayerMessage
        self._message_handlers["layersprocessor.proto.GCodePrefix"] = self._onGCodePrefixMessage
        self._message_handlers["layersprocessor.proto.PrintTimeMaterialEstimates"] = self._onPrintTimeMaterialEstimates
        self._message_handlers["layersprocessor.proto.SlicingFinished"] = self._onSlicingFinishedMessage
    ...
    def _onSocketError(self, error: Arcus.Error) -> None:
        ...
        if error.getErrorCode() == Arcus.ErrorCode.BindFailedError and self._process_mesh_job is not None:
            self._process_mesh_job.setIsCancelled(False)
    ...
    def _createSocket(self, protocol_file: str = None) -> None:
        if not protocol_file:
            ...
            if not plugin_path:
                ...
                return
            protocol_file = os.path.abspath(os.path.join(plugin_path, "LayersProcessor.proto"))
