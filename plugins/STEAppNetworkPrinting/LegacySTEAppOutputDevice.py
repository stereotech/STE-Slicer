...
from .LegacySTEAppPrinterOutputController import LegacySTEAppPrinterOutputController
...
class LegacySTEAppOutputDevice(NetworkedPrinterOutputDevice):
    def __init__(self, device_id, address: str, properties, parent=None) -> None:
        ...
        self._output_controller = LegacySTEAppPrinterOutputController(self)
    ...
    def _onAuthenticationStateChanged(self):
        ...
        if self._authentication_state == AuthState.Authenticated:
            ...
            self.setConnectionText(
                i18n_catalog.i18nc("@info:status", "Connected over the network. No access to control the printer."))
    ...
    def _sendMaterialProfiles(self):
        ...
        for container in ContainerRegistry.getInstance().findInstanceContainers(type="material"):
            try:
                ...
                self.postForm("materials", "form-data; name=\"file\";filename=\"%s\"" % file_name, xml_data.encode(),
                              on_finished=None)
    ...
    def requestWrite(self, nodes: List[SceneNode], file_name: Optional[str] = None, limit_mimetypes: bool = False,
        ...
        if errors:
            ...
            SteSlicerApplication.getInstance().messageBox(
                i18n_catalog.i18nc("@window:title", "Mismatched configuration"),
                text,
                informative_text,
                detailed_text,
    ...
    def _checkForErrors(self):
        ...
        for index, extruder in enumerate(self.activePrinter.extruders):
            ...
            if extruder.hotendID == "":
                # No Printcore loaded.
                errors.append(i18n_catalog.i18nc("@info:status", "No Printcore loaded in slot {slot_number}".format(
                    slot_number=index + 1)))
            ...
            if index < len(print_information.materialLengths) and print_information.materialLengths[index] != 0:
                ...
                if extruder.activeMaterial is None:
                    # No active material
                    errors.append(i18n_catalog.i18nc("@info:status", "No material loaded in slot {slot_number}".format(
                        slot_number=index + 1)))
        return errors
            ...
            if index < len(print_information.materialLengths) and print_information.materialLengths[index] != 0:
                ...
                if variant:
                    if variant.getName() != extruder.hotendID:
                        warnings.append(i18n_catalog.i18nc("@label",
                                                           "Different PrintCore (Cura: {cura_printcore_name}, Printer: {remote_printcore_name}) selected for extruder {extruder_id}".format(
                ...
                if local_material:
                    if extruder.activeMaterial.guid != local_material.getMetaDataEntry("GUID"):
                        Logger.log("w", "Extruder %s has a different material (%s) as Cura (%s)", index + 1,
                                   extruder.activeMaterial.guid, local_material.getMetaDataEntry("GUID"))
                        warnings.append(i18n_catalog.i18nc("@label",
                                                           "Different material (Cura: {0}, Printer: {1}) selected for extruder {2}").format(
                            local_material.getName(), extruder.activeMaterial.name, index + 1))
                else:
    ...
    def _checkAuthentication(self):
        Logger.log("d", "Checking if authentication is correct for id %s and key %s", self._authentication_id,
    ...
    def _onGetPrinterDataFinished(self, reply):
        ...
        if status_code == 200:
            ...
            if not self._printers:
                ...
                self._printers = [PrinterOutputModel(output_controller=self._output_controller,
