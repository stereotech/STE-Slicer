...
SettingItem
{
    ...
    contents: Rectangle
    {
        ...
        TextInput
        {
            ...
            maximumLength: (definition.type == "str" || definition.type == "[int]" || definition.type == "[float]") ? -1 : 10;
            clip: true; //Hide any text that exceeds the width of the text box.

            validator: RegExpValidator { regExp: (definition.type == "[int]") ? /^\[?(\s*-?[0-9]{0,9}\s*,)*(\s*-?[0-9]{0,9})\s*\]?$/ : (definition.type == "int") ? /^-?[0-9]{0,10}$/ : (definition.type == "[float]") ? /^\[?(\s*-?[0-9]{0,9}[.,]?[0-9]{0,10}\s*,)*(\s*-?[0-9]{0,9}[.,]?[0-9]{0,10})\s*\]?$/ : (definition.type == "float") ? /^-?[0-9]{0,9}[.,]?[0-9]{0,10}$/ : /^.*$/ } // definition.type property from parent loader used to disallow fractional number entry
            Binding
            {
                target: input
                property: "text"
