...
from steslicer.PrinterOutputDevice import PrinterOutputDevice
...
if TYPE_CHECKING:
    ...
    from steslicer.Machines.QualityGroup import QualityGroup
...
class MachineManager(QObject):
    def __init__(self, application: "SteSlicerApplication", parent: Optional["QObject"] = None) -> None:
        ...
        self._application.getPreferences().addPreference("steslicer/active_machine", "")
    ...
    def _onGlobalContainerChanged(self) -> None:
        ...
        if self._global_container_stack:
            self._application.getPreferences().setValue("steslicer/active_machine", self._global_container_stack.getId())
    ...
    def applyRemoteConfiguration(self, configuration: ConfigurationModel) -> None:
        ...
        if self.hasUserSettings and self._application.getPreferences().getValue("steslicer/active_mode") == 1:
    ...
    def setQualityGroup(self, quality_group: "QualityGroup", no_dialog: bool = False, global_stack: Optional["GlobalStack"] = None) -> None:
        ...
        # See if we need to show the Discard or Keep changes screen
        if not no_dialog and self.hasUserSettings and self._application.getPreferences().getValue("steslicer/active_mode") == 1:
            self._application.discardOrKeepProfileChanges()
