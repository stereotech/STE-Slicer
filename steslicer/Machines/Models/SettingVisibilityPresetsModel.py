...
class SettingVisibilityPresetsModel(QObject):
    ...
    def __init__(self, preferences, parent = None):
        ...
        basic_item = self.getVisibilityPresetById("spiral")
        basic_visibile_settings = ";".join(basic_item.settings)

        self._preferences = preferences

        # Preference to store which preset is currently selected
        self._preferences.addPreference("steslicer/active_setting_visibility_preset", "spiral")

        # Preference that stores the "custom" set so it can always be restored (even after a restart)
        self._preferences.addPreference("steslicer/custom_visible_settings", basic_visibile_settings)
        self._preferences.preferenceChanged.connect(self._onPreferencesChanged)

        self._active_preset_item = self.getVisibilityPresetById(self._preferences.getValue("steslicer/active_setting_visibility_preset"))
    ...
    def setActivePreset(self, preset_id: str) -> None:
        ...
        if need_to_save_to_custom:
            ...
            if current_visibility_string:
                self._preferences.setValue("steslicer/custom_visible_settings", current_visibility_string)

        new_visibility_string = ";".join(preset_item.settings)
        if preset_id == "custom":
            # Get settings from the stored custom data
            new_visibility_string = self._preferences.getValue("steslicer/custom_visible_settings")
        ...
        self._preferences.setValue("steslicer/active_setting_visibility_preset", preset_id)
    ...
    def _onPreferencesChanged(self, name: str) -> None:
        ...
        if matching_preset_item is None:
            ...
            if self._active_preset_item is None or self._active_preset_item.presetId == "custom":
                ...
                self._preferences.setValue("steslicer/custom_visible_settings", visibility_string)
            else:
        ...
        if self._active_preset_item is None or self._active_preset_item.presetId != item_to_set.presetId:
            ...
            self._preferences.setValue("steslicer/active_setting_visibility_preset", self._active_preset_item.presetId)
            self.activePresetChanged.emit()
